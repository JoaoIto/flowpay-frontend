...
A nova engine V4 (`@tailwindcss/vite`) retira a necessidade de arquivos pesados de configuração (`tailwind.config.js`). As cores de design premium do projeto (glassmorphism via *backdrop-blur*, tons slate/emerald) são definidas como tokens CSS puros em `index.css`.
